...
class TestRegimenContributivo:
    def test_activo_aplica_cuota_moderadora(self) -> None:
        r = validar_afiliacion(Regimen.CONTRIBUTIVO, afiliacion_activa=True)
        assert r.activa
        assert r.cubierto
    ...
    def test_nivel_desconocido_cae_al_nivel_1(self) -> None:
        ...
        assert f"{CUOTA_MODERADORA_POR_NIVEL[1]:,.0f}" in r.mensaje


class TestRegimenSubsidiado:
    def test_activo_aplica_copago(self) -> None:
        r = validar_afiliacion(Regimen.SUBSIDIADO, afiliacion_activa=True)
...
class TestParticular:
    def test_nunca_tiene_copago(self) -> None:
        r = validar_afiliacion(Regimen.PARTICULAR, afiliacion_activa=True)
    ...
    def test_el_flag_de_afiliacion_es_irrelevante_para_un_particular(self) -> None:
        ...
        activo = validar_afiliacion(Regimen.PARTICULAR, afiliacion_activa=True)
        inactivo = validar_afiliacion(Regimen.PARTICULAR, afiliacion_activa=False)
...
class TestSoat:
    def test_cubre_totalmente_y_no_genera_copago(self) -> None:
        r = validar_afiliacion(Regimen.SOAT, afiliacion_activa=True)
        assert r.cubierto
        assert not r.requiere_copago
...
class TestAfiliacionInactiva:
    ...
    def test_cae_a_tarifa_particular(self, regimen: Regimen) -> None:
        r = validar_afiliacion(regimen, afiliacion_activa=False)
        assert not r.activa
...
class TestNoBloqueaAgendamiento:
    ...
    def test_ninguna_combinacion_impide_agendar(
        ...
        r = validar_afiliacion(regimen, activa, nivel_cuota_moderadora=nivel)
...
class TestTarifas:
    ...
    def test_toda_especialidad_tiene_tarifa(self, especialidad: Especialidad) -> None:
        assert str(especialidad) in TARIFA_PARTICULAR
        assert tarifa_base(str(especialidad)) > Decimal("0")

    def test_especialidad_desconocida_usa_la_tarifa_por_defecto(self) -> None:
        assert tarifa_base("cirugia_espacial") == TARIFA_PARTICULAR_DEFECTO

    def test_endodoncia_es_la_mas_costosa(self) -> None:
        assert tarifa_base("endodoncia") == max(TARIFA_PARTICULAR.values())
